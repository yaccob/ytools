...
def validate(schemafile, datafiles, encoding='utf-8'):
    schema = yaml.load("".join(fileinput.input(schemafile)), Loader=yaml.FullLoader)
    documents = yaml.load_all("".join(fileinput.input(datafiles, openhook=fileinput.hook_encoded(encoding))), Loader=yaml.FullLoader)
    for filename in datafiles:
        with open(filename) as infile:
            for document in yaml.load_all(infile, Loader=yaml.FullLoader):
                try:
...
def dump(datafiles, path='$', format='yaml', yaml_options=optiondefaults['yaml'], json_options=optiondefaults['json'], encoding='utf-8'):
    ...
    documents = yaml.load_all("".join(fileinput.input(datafiles, openhook=fileinput.hook_encoded(encoding))), Loader=yaml.FullLoader)
    formatoptions = dict(yaml.load(optiondefaults[format], Loader=yaml.FullLoader), **yaml.load(encoders[format]["kwargs"], Loader=yaml.FullLoader))
